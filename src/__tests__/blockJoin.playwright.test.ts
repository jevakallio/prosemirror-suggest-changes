...
test.describe("Block Join E2E - Real Keyboard Events", () => {
  ...
  test.describe("Paragraph: Enter then Backspace", () => {
    test("should revert document to original state", async ({ page }) => {
      ...
      await page.evaluate(() => { window.pmEditor.setCursorToEnd(); });
      ...
      const afterEnterDoc = await page.evaluate(() => window.pmEditor.getDocJSON());
      const afterEnterCursor = await page.evaluate(() => window.pmEditor.getCursorInfo());
      ...
      const transactions = await page.evaluate(() => window.pmEditor.getTransactions());
  ...
  test.describe("Paragraph: Enter twice then Backspace twice", () => {
    test("should revert through both suggestions", async ({ page }) => {
      ...
      const afterSecondEnter = await page.evaluate(() => window.pmEditor.getState());
      ...
      const afterFirstBackspace = await page.evaluate(() => window.pmEditor.getState());
  ...
  test.describe("Paragraph: Enter then Delete (from first block)", () => {
    test("should revert when Delete pressed from end of first block", async ({ page }) => {
      ...
      await page.evaluate(() => { window.pmEditor.setCursorToEndOfBlock(0); });

      const cursorInfo = await page.evaluate(() => window.pmEditor.getCursorInfo());
  ...
  test.describe("Paragraph: Enter then Delete (at block boundary)", () => {
    test("should revert when Delete pressed at exact block boundary", async ({ page }) => {
      ...
      // Move cursor to end
      await page.evaluate(() => { window.pmEditor.setCursorToEnd(); });
      const endPos = (await page.evaluate(() => window.pmEditor.getState())).cursorFrom;
      ...
      await page.evaluate((pos) => { window.pmEditor.setCursorToPosition(pos); }, endPos);

      const boundaryInfo = await page.evaluate(() => window.pmEditor.getCursorInfo());
  ...
  test.describe("Edge Cases", () => {
    test("should handle Enter at middle of text then Backspace", async ({ page }) => {
      ...
      await page.evaluate(() => { window.pmEditor.setCursorToPosition(6); });

      const cursorBefore = await page.evaluate(() => window.pmEditor.getCursorInfo());
    ...
    test("should handle multiple rapid Enter/Backspace sequences", async ({ page }) => {
      ...
      for (let i = 1; i <= 3; i++) {
        ...
        const afterBackspace = await page.evaluate(() => window.pmEditor.getState());
