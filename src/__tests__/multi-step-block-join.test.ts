...
describe("Multi-Step Block Join Operations", () => {
  it("should handle normal replace step before block join", () => {
    ...
    assert(
      ...
      "Document should start with ZWSP characters",
    );
    ...
    assert(
      ...
      "No ZWSP characters should remain after block join",
    );
  ...
  it("should handle two block splits and joins in one transaction", () => {
    ...
    assert(
      !finalState.doc.textContent.includes("\u200B"),
      "No ZWSP characters should remain after both block joins",
    );
  ...
  it("should handle block split with intermediate text changes", () => {
    ...
    const expected = testBuilders.doc(
      testBuilders.paragraph(
        "test",
        testBuilders.insertion({ id: 1 }, "INSERTED "),
        " paragraph",
      ),
    );
  ...
  it("should handle nested block operations", () => {
    ...
    assert(
      !finalState.doc.textContent.includes("\u200B"),
      "No ZWSP characters should remain after nested joins",
    );
